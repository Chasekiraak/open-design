...
- `apps/codex-plugin` is the Codex host shell source; it owns the Codex manifest/skills, Node-independent platform bootstrap, stdio MCP entry, runtime acquisition, and handoff. It shares packaged/desktop's channel, namespace, and final daemon data root, while keeping environment, runtime, update, cache, log, state, lease, exit, and heartbeat lifecycle below `namespaces/<namespace>/codex-plugin/`.
- `packages/distribution-proto` owns shell-neutral channel/namespace/data coordinates plus reusable runtime pointer/binding/lease primitives; those primitives do not imply cross-shell state sharing. `packages/codex-plugin-proto` layers Codex bootstrap argv/env, isolated lifecycle paths, acquisition/control manifests, environment artifacts, handoff descriptors, and ready-message validation on top.
