...
- `apps/codex-plugin` is the Codex host shell source; it owns the Codex manifest/skills, platform-targeted plugin-native execution carrier, stdio MCP entry, runtime acquisition, and handoff. It shares packaged/desktop's channel, namespace, and final daemon data root, while keeping runtime, update, cache, log, state, lease, exit, and heartbeat lifecycle below `namespaces/<namespace>/codex-plugin/`. The carrier, MCP bundle, skills, and identity update only as one native plugin artifact.
- `packages/distribution-proto` owns shell-neutral channel/namespace/data coordinates plus reusable runtime pointer/binding/lease primitives; those primitives do not imply cross-shell state sharing. `packages/codex-plugin-proto` layers Codex launcher args/runtime env names, platform targets, isolated lifecycle paths, acquisition/control manifests, handoff descriptors, and ready-message validation on top.
...
- Keep root scripts reserved for true repo-level checks and tools control-plane entrypoints: `pnpm guard`, `pnpm typecheck`, `pnpm tools-codex`, `pnpm tools-dev`, `pnpm tools-pack`, and `pnpm tools-serve`.
