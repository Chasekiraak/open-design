...
export const OPEN_DESIGN_CLOUD_PLUGIN_ID = 'open-design-cloud';
...
export interface ExternalPluginContext {
  id: typeof OPEN_DESIGN_CLOUD_PLUGIN_ID;
  version: string;
...
export interface ExternalPluginRunAnalyticsHints {
  ...
  externalPluginId: typeof OPEN_DESIGN_CLOUD_PLUGIN_ID;
...
export function validateExternalPluginContext(
  ...
  if (input.id !== OPEN_DESIGN_CLOUD_PLUGIN_ID) {
    throw pluginContractError(
      `id must be ${OPEN_DESIGN_CLOUD_PLUGIN_ID}`,
    );
  }
  if (
  ...
  return {
    id: OPEN_DESIGN_CLOUD_PLUGIN_ID,
