...
//   - Only projects this daemon has locally bound as team projects
//     (`workspace_projects` visibility='team') are pulled. A share the member
//     never materialized costs zero requests, same principle as the comment
//     dirty-mark path.
...
export interface ProactiveContentPullDeps {
  ...
  pullSharedProject: (projectId: string) => Promise<ProactiveContentPullOutcome>;
...
export function createProactiveContentPull(
  ...
  /** Last hub version a successful pull materialized, per project. */
  const pulledVersions = new Map<string, number>();
  /** In-flight pull per project; racing events await it instead of stacking. */
  const inFlight = new Map<string, Promise<void>>();

  async function shouldPull(event: ProactiveContentPullEvent): Promise<boolean> {
    const projectId = event.projectId;
    if (!projectId) return false;
    const binding = deps.getLocalBinding(projectId);
    if (!binding || binding.visibility !== 'team') return false;
    if (event.workspaceId && binding.workspaceId !== event.workspaceId) return false;
    const identity = await deps.getWorkspaceIdentity();
    if (!identity) return false;
    if (identity.workspaceId !== binding.workspaceId) return false;
    ...
    if (owner === identity.workspaceMemberId) return false;
    return true;
  }

  async function runPull(projectId: string): Promise<void> {
    const run = (async () => {
      try {
        const outcome = await deps.pullSharedProject(projectId);
        ...
        const cursor = pulledVersions.get(projectId);
        if (cursor == null || outcome.version > cursor) {
          pulledVersions.set(projectId, outcome.version);
        }
      } catch (error) {
  ...
  return {
    async handleContentChanged(event: ProactiveContentPullEvent): Promise<void> {
      try {
        if (!(await shouldPull(event))) return;
        const projectId = event.projectId!;
