...
export const zhTW: Dict = {
  ...
  'settings.codexPathRepairHint': '目前儲存的 Codex 路徑不適合繼續使用。',
  'settings.codexPathUseDetected': '使用偵測到的 Codex',
  'settings.codexPathClearCustom': '清除自訂路徑',
  'settings.codexPathConfiguredSuccess': '本次測試使用的是已設定的 Codex 路徑：{path}。',
  'settings.codexPathInvalidFallback': '已設定的 Codex 路徑無效或不可執行：{configuredPath}。本次測試改用 PATH 中的 Codex CLI：{detectedPath}。建議更新 CODEX_BIN 或清除自訂路徑。',
  'settings.codexPathFailedFallback': '已設定的 Codex 路徑啟動失敗：{configuredPath}。本次測試改用 PATH 中的 Codex CLI：{detectedPath}。建議更新 CODEX_BIN 或清除自訂路徑。',
