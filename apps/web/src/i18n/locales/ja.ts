...
export const ja: Dict = {
  ...
  'settings.codexPathRepairHint': '保存されている Codex のパスは、このテストで使用すべきバイナリではありません。',
  'settings.codexPathUseDetected': '検出された Codex を使用',
  'settings.codexPathClearCustom': 'カスタムパスをクリア',
  'settings.codexPathConfiguredSuccess': 'このテストでは設定済みの Codex パスを使用しました：{path}。',
  'settings.codexPathInvalidFallback': '設定された Codex パスが無効か実行できません：{configuredPath}。このテストでは PATH 上の Codex CLI（{detectedPath}）を使用しました。CODEX_BIN を更新するか、カスタムパスをクリアしてください。',
  'settings.codexPathFailedFallback': '設定された Codex パスの起動に失敗しました：{configuredPath}。このテストは PATH 上の Codex CLI（{detectedPath}）で成功しました。CODEX_BIN を更新するか、カスタムパスをクリアしてください。',
