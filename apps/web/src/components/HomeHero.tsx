...
export const HomeHero = forwardRef<HomeHeroHandle, Props>(function HomeHero(
  ...
  const heroCapabilities = useMemo(() => heroCapabilitiesForHome(onboardingRole), [onboardingRole]);
  const activeHeroCapability = heroCapabilities[heroCapabilityIndex % Math.max(heroCapabilities.length, 1)] ?? null;
  const heroCapabilityCarouselRunning = heroCapabilities.length > 1;
  ...
  const placeholderCarouselActive = carouselActive;
  ...
  const templateChips = useMemo(
    () => {
      const catalog = orderedCreateChips().filter((chip) => chip.action.kind === 'apply-scenario');
      const byId = new Map(catalog.map((chip) => [chip.id, chip]));
      return orderHomeTemplateIds(onboardingRole, catalog.map((chip) => chip.id))
        .map((id) => byId.get(id))
      ...
      <h1 className="home-hero__title">
        ...
        <span className="home-hero__capability-slot">
          {activeHeroCapability ? (
            <span
              key={activeHeroCapability.id}
              className="home-hero__capability-value"
            >
              <button
                type="button"
      ...
      >
        ...
        <div className="home-hero__prompt-surface">
          <div ref={promptEditorRef} className="home-hero__prompt-editor home-hero__lexical">
            ...
            <PlaceholderCarousel
              ...
              label={t('homeHero.carousel.examplePrefix')}
        ...
        <div className="home-hero__input-foot">
          ...
          <div className="home-hero__foot-left">
            ...
            />
            {selectedDesignSystem && onDesignSystemChange ? (
              <span
                className="home-hero__selected-design-system"
                data-testid="home-hero-selected-design-system"
              >
                <Icon
                  name="palette"
                  size={14}
                ...
                <button
                  type="button"
                  className="home-hero__selected-design-system-clear od-tooltip"
                  aria-label={`${t('common.clear')} ${selectedDesignSystem.title}`}
...
interface RailGroupProps {
  group: ChipGroup;
  /** Optional catalog override for role-aware Home ordering. */
...
function RailGroup({
  ...
  const chips = suppliedChips ?? catalogChips;
