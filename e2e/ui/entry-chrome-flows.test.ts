...
test('[P1] entry top navigation matches the current home tab structure', async ({ page }) => {
  ...
  // Settings is the rail's own control, never a duplicate of the nav group's
  // destinations. #5971 cut the footer chip, so signed out it is the rail nav
  // item `entry-nav-settings`; the footer no longer carries any destination.
